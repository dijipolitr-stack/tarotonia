...
function getClient() {
  if (!client) {
    ...
    const options = {
      // Fail fast instead of hanging the function if Redis is unreachable.
      maxRetriesPerRequest: 3,
      connectTimeout: 10000,
      enableReadyCheck: true
    };

    // Managed Redis (Vercel / Upstash) requires TLS even when the connection
    // string uses the redis:// scheme. Enable TLS with the correct SNI host
    // for any non-local host so the handshake succeeds.
    const isLocal = /@(localhost|127\.0\.0\.1|\[::1\])(:|$)/.test(url);
    if (url.startsWith('rediss://') || !isLocal) {
      let host;
      try {
      ...
      options.tls = host ? { servername: host } : {};
    }
