...
function getClient() {
  if (!client) {
    // Prefer the well-known names, but fall back to ANY env var holding a
    // redis://-style connection string. This keeps the app working no matter
    // which provider/integration is connected or how it names its variable.
    const url =
      process.env.KV_REDIS_URL ||
      process.env.REDIS_URL ||
      process.env.KV_URL ||
      Object.values(process.env).find(
        (v) => typeof v === 'string' && /^rediss?:\/\//.test(v)
      );

    if (!url) {
      throw new Error('Redis bağlantı adresi bulunamadı (redis:// env değişkeni yok)');
    }
